...
* `HVWiresPolarization`: `'Reverse'` for reverse bias (V in [0, 500]); `'Forward'` for forward bias (V in [-500, 0])
...
A note about the "Hexaboard/ROC Version" in the Hexaboard setup window. Preseries hexaboards were all labeled `03`, but preproduction and production hexaboard serial numbers now have changed the meaning of these two digits. The first digit is now the hexaboard version (almost always `4`) and the second is the ROC version (`X` for V3, `2`, `B`, or `4` for V3b, `C` for V3c). The GUI will interpret the radio buttons accordingly; ensure the display shows the correct serial number.
