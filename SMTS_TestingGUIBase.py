...
modulesetup = [[sg.Radio('Live Module', 1, key="-IsLive-", enable_events=True), sg.Radio('Hexaboard', 1, key='-IsHB-', enable_events=True)],
               ...
               [sg.Text('ROC Version: '),
                sg.Radio('X', 7, key='-Preseries-', default=True, enable_events=True),
                sg.Radio("V3B 2", 7, key='-V3b-2-', enable_events=True),
                sg.Radio("V3B B", 7, key='-V3b-B-', enable_events=True),
                sg.Radio("V3B 4", 7, key='-V3b-4-', enable_events=True),
                sg.Radio('V3C', 7, key='-V3c-', enable_events=True),
                sg.Radio('V3D', 7, key='-V3d-', enable_events=True),
                sg.Radio('V3E', 7, key='-V3e-', enable_events=True)],
...
def toggle_module_setup(enabled):
    keys = ['-DEBUG-MODE-', '-IsLive-', '-IsHB-', '-LD-', '-HD-', '-Full-', '-Top-', '-Bottom-', '-Left-', '-Right-', '-Five-', '-120-', '-200-', '-300-',
            '-Ti-', '-CF-', '-CuW-', '-Preseries-', '-V3b-2-', '-V3b-B-', '-V3b-4-', '-V3c-', '-V3d-', '-V3e-', '-HB-Manufacturer-', '-Module-Index-', '-FPGAHostname-', 'Configure Test Stand',
...
while True:
    ...
    if values['-Scanned-QR-Code-'] != '':
        ...
        if values['-IsLive-']:
            ...
            if len(serialsections[2]) == 4:
                ...
                elif serialsections[2][3] == 'D': basewindow['-V3d-'].update(value=True)
                elif serialsections[2][3] == 'E': basewindow['-V3e-'].update(value=True)
                #else:
            ...
            if serialsections[2][1:3] == '03':
                ...
                elif serialsections[2][2] == 'D':
                    basewindow['-V3d-'].update(value=True)
                elif serialsections[2][2] == 'E':
                    basewindow['-V3e-'].update(value=True)
            #else:
            #    basewindow['-Scanned-QR-Code-'].update(value='')
            #    continue
    ...
    if values['-IsLive-']:
        ...
        elif values['-V3d-']: minortype[3] = 'C'
        elif values['-V3e-']: minortype[3] = 'C'
        ...
        if values['-Standard-Test-']:
            ...
            if modulestatus == 'Completely Bonded' or modulestatus == 'Frontside Bonded' or modulestatus == 'Bonds Reworked':
                ...
                try:
                    ...
                    _, _, pxoffset, pyoffset, pangoffset, _, _, mxoffset, myoffset, mangoffset, _, _ = assembly_info(moduleserial)
